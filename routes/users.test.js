...
const {
  ...
  commonAfterEach,
  commonAfterAll,
  u1Token, u2Token, adminToken, testJobIds
} = require("./_testCommon");
...
describe("POST /users", function () {
  ...
  test("fails: unauth for non-admin users", async function () {
    const resp = await request(app)
        .post("/users")
        .send({
          username: "u-new",
  ...
  test("fails: bad request if invalid data", async function () {
    const resp = await request(app)
        .post("/users")
        .send({
          username: "u-new",
...
describe("GET /users/:username", function () {
  test("works for admin users", async function () {
    await request(app).post(`/users/u1/jobs/${testJobIds[1]}`)
                      .set("authorization", `Bearer ${u1Token}`);
  ...
  test("fails: unauth for non-admin/incorrect user", async function () {
    const resp = await request(app)
        .get(`/users/u1`)
  ...
  test("fails: not found if user not found", async function () {
    const resp = await request(app)
        .get(`/users/nope`)
        .set("authorization", `Bearer ${adminToken}`);
...
describe("POST /users/:username/jobs/:id", () => {
  test("applying works for correct user", async () => {
    const resp = await request(app)
  ...
  test("applying works for admin user", async () => {
    const resp = await request(app)
  ...
  test("not found if user doesn't exist", async function () {
    const resp = await request(app)
        .post(`/users/unknown/jobs/${testJobIds[0]}`)
        .set("authorization", `Bearer ${adminToken}`);
    expect(resp.statusCode).toEqual(404);
  });
